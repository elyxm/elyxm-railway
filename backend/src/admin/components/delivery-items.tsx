...
const DeliveryItems = ({ delivery }: { delivery: DeliveryDTO }) => {
  ...
  return (
    <>
      ...
      >
        <span className="border-b border-ui-fg-muted border-dotted font-medium min-w-2">{delivery.items.length}</span>
      </div>
      {hovered && (
...
const DeliveryItem = ({ item }: { item: Record<string, any> }) => {
  ...
  return (
    <div className="flex items-center gap-4">
      <img src={thumbnail} alt={item.title} className="w-16 h-16 rounded-md" width={64} height={64} />
