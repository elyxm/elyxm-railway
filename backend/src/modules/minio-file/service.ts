...
class MinioFileProviderService extends AbstractFileProviderService {
  ...
  constructor({ logger }: InjectedDependencies, options: MinioFileProviderOptions) {
    ...
    this.initializeBucket().catch((error) => {
      this.logger_.error(`Failed to initialize MinIO bucket: ${(error as Error).message}`);
    });
  }
    ...
    try {
      ...
      if (!bucketExists) {
        ...
        try {
          ...
          this.logger_.warn(`Failed to update policy for existing bucket: ${(policyError as Error).message}`);
        }
      }
    } catch (error) {
      this.logger_.error(`Error initializing bucket: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  async upload(file: ProviderUploadFileDTO): Promise<ProviderFileResultDTO> {
    ...
    try {
      ...
      this.logger_.error(`Failed to upload file: ${(error as Error).message}`);
      throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to upload file: ${(error as Error).message}`);
    }
  }
  ...
  async delete(fileData: ProviderDeleteFileDTO): Promise<void> {
    ...
    try {
      ...
      this.logger_.warn(`Failed to delete file ${fileData.fileKey}: ${(error as Error).message}`);
    }
  }
  ...
  async getPresignedDownloadUrl(fileData: ProviderGetFileDTO): Promise<string> {
    ...
    try {
      ...
      this.logger_.error(`Failed to generate presigned URL: ${(error as Error).message}`);
